class Intcode
  ...
  def initialize(mem, pos: 0, relative_base: 0, sparse: false, valid_ops: nil)
  ...
  def self.disas(mem, addrs_run: nil)
    ...
    while (opcode = mem[pos])
      if addrs_run && !addrs_run.has_key?(pos) || !(op = OPS[opcode % 100])
        ...
        pos += 1 until pos >= mem.size || (!addrs_run || addrs_run.has_key?(pos)) && (OPS[(op = mem[pos] || 0) % 100] && op <= 22208)
