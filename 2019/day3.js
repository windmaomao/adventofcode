...
const getPosArr = line => {
  ...
  return parts.reduce((acc, ins) => {
    ...
    switch (char) {
      case 'R':
        acc.h.push({y: o.y, x: [o.x, o.x + num], d: 1, l: o.l, s: num })
        o.x = o.x + num
        break
      case 'U':
        acc.v.push({ x: o.x, y: [o.y, o.y + num], d: 1, l: o.l, s: num })
        o.y = o.y + num
        break
      case 'L':
        acc.h.push({ y: o.y, x: [o.x - num, o.x], d: -1, l: o.l, s: num })
        o.x = o.x - num
        break
      case 'D':
        acc.v.push({ x: o.x, y: [o.y - num, o.y], d: -1, l: o.l, s: num })
        o.y = o.y - num
        break
    }
    return acc  
...
const l1 = getPosArr(lines[0])
const l2 = getPosArr(lines[1])
// console.log(l1,l2)

const findInter = (h, v) => {
  if (h.y <= v.y[1] && h.y >= v.y[0] && v.x <= h.x[1] && v.x >= h.x[0]) {
    const pY = (v.d < 0) ? h.y - v.y[0] : v.y[1] - h.y
    const pX = (h.d < 0) ? v.x - h.x[0] : h.x[1] - v.x
    return { x: v.x, y: h.y, l: h.l - pX + v.l - pY }
  }
  return false
}

const getInters = (l1, l2) => {
  const inters = []
  l1.h.forEach(h => {
    l2.v.forEach(v => {
      const found = findInter(h, v)
      if (found) inters.push(found)
    }) 
  })
  l1.v.forEach(v => {
    l2.h.forEach(h => {
...
// 266
const ints = getInters(l1, l2)
let len = 10000000
const mLen = ints.reduce((acc, p) => {
  const tmp = p.x * p.x + p.y * p.y
  if (tmp < len && tmp > 0) { 
    console.log(p)
    len = tmp
...
console.log('Day 3-1:', mLen)

let len2 = 10000000
const sLen = ints.reduce((acc, p) => {
  if (p.l < len2 && p.l > 0) {
    console.log(p)
    len2 = p.l
  }
